...
void fprint_goal(FILE *, struct goalrec *,
		module, unsigned long, unsigned long, int);
void print_priority(long, struct trace_info_rec *);
void print_goal(struct goalrec *, module,
		struct trace_info_rec *);
void trace_fflush(struct trace_info_rec *);
static void trace_printf();
void trace_fgets(char *, int, struct trace_info_rec *);
...
static int mod_compare_names(x, y)
     struct mod_table_entry *x, *y;
{
  return strcmp((char *)x->name, (char *)y->name);
}

static int mod_compare_addr(x, y)
     struct mod_table_entry **x, **y;
{
  return (long)(*x)->func - (long)(*y)->func;
}

static int pred_compare_names(x, y)
     ...
     const struct predicate *pred;
{
  ...
  return 0;
  */
  // from klic-3.003-extio-shared:
   for (mid = low; mid < high; mid++) {
     // fprintf(stderr, "get_mte:%p %p %p\n", (module)mod_index[mid]->func, pred, mod); /* ueda */
     if ((module)mod_index[mid]->func == mod) break;
   }
   if (mid == high) {
          fprintf(stderr, "get_mte(): not found(bug). TODO\n");  // for debug
     return 0;
...
static void enter_pred_hash(pred, pte)
     const struct predicate *pred;
     struct pred_table_entry *pte;
{
 ...
 retry:
  ...
  if (!Leaping(info) || Spying(info)) {
    ...
    if (Enabled(info,Call)) {
      ...
      for (;;) {
#ifdef PARALLEL
	trace_printf(info, "%d:%9d CALL: ", my_node, parent_seq, 0, 0, 0, 0);
#else
	trace_printf(info, "%4d CALL: ", parent_seq, 0, 0, 0, 0, 0);
#endif
     ...
     q *allocp;
{
  ...
  if (Enabled(info, Reduce) &&
    ...
    for (;;) {
#ifdef PARALLEL
      trace_printf(info, "%d:%9d REDU: ", my_node, parent_seq, 0, 0, 0, 0, 0, 0);
#else
      trace_printf(info, "%4d REDU: ", parent_seq, 0, 0, 0, 0, 0, 0);
#endif
      print_goal(parent, 0, info);
      if (subgoals != 0) {
        trace_printf(info, " :-", 0, 0, 0, 0, 0, 0);
	k = 0;
	for (qt = qp;
	     qt != before;
	     qt = qt->next, k++) {
	  trace_printf(info, "\n%4d%4d%c%c",
		  ...
		  (k < SUBGOAL_MAX && subgoal_trace[k] ? '+' : '-' ), 0, 0);
	  print_goal(qt, (module)parent->pred->func, info);
	}
	for (tr = trace_enqueued_goals;
	     tr != 0 && k < SUBGOAL_MAX;
	     tr = tr->next, k++) {
		  ...
		  (k < SUBGOAL_MAX && is_resumed[k] ? '#' : '*'),
		  (k < SUBGOAL_MAX && subgoal_trace[k] ? '+' : '-' ), 0, 0);
	  print_goal(tr->g, (module)parent->pred->func, info);
     ...
     int spontaneous;
{
  ...
  if (Enabled(info,Susp) && (!Leaping(info) || Spying(info))) {
    ...
    for (;;) {
#ifdef PARALLEL
      trace_printf(info, "%d:%9d SUSP: ", my_node, seq, 0, 0, 0, 0);
#else
      trace_printf(info, "%4d SUSP: ", seq, 0, 0, 0, 0, 0);
#endif
      print_goal(qp, 0, info);
     ...
     struct goalrec *qp;
{
  ...
  if (Enabled(info,Fail)) {
    ...
    for (;;) {
#ifdef PARALLEL
      trace_printf(info, "%d:%9d FAIL: ", my_node, seq, 0, 0, 0, 0);
#else
      trace_printf(info, "%4d FAIL: ", seq, 0, 0, 0, 0, 0);
#endif
      print_goal(qp, 0, info);
     ...
     struct goalrec *qp;
{
  ...
  if (Enabled(info,Fail)) {
    for (;;) {
#ifdef PARALLEL
      trace_printf(info, "%d:%9d PSUS: ", my_node, seq, 0, 0, 0, 0);
      print_goal(qp, 0, info);
#else
      trace_printf(info, "%4d PSUS: ", seq, 0, 0, 0, 0, 0);
      print_goal(qp, 0, info);
#endif
      /* to access module table entry (mte), it's necessary to access the 
         original predicate structure rather than the predicate structure 
         of the trace trigger predicate. */ /* ueda */
      // if (!fail_port_command(qp, get_mte(qp->pred), &info)) break;
      struct predicate *real_pred = (struct predicate *)
	((unsigned long)(qp->args[qp->pred->arity-ExtraArgs]) - CONS);
      // fprintf(stderr,"...\n pred=%p %d %p\n", qp->pred,
      //                qp->pred->arity-ExtraArgs, real_pred); /* ueda */
      if (!fail_port_command(qp, get_mte(real_pred), &info)) break;
    }
  }
  TRACE_DETACHIO();
}
...
static int trace_parse_name(np)
     char *np;
{
  if (*clp == '\0') return 0;
...
static int trace_parse_predicate_name(mod, pred, arity, curmod, info)
     const struct mod_table_entry **mod;
...
*/
{
 ...
 pred_found:
  ...
  if (trace_parse_int((long *)arity) != 1) {
...
static void common_port_command(command, pred, curmod, g, info)
     enum tracer_command command;
     const struct predicate *pred;
     struct mod_table_entry *curmod;
     struct goalrec *g;
...
#define SetSpyOrTrace(x, y) \
  ...
  set_reset:
	...
	trace_printf(*info, "\t%s %s on all predicates in module %s\n",
		     spy_trace[is_spy], set_reset[set_flag], mte->name, 0, 0, 0);
	break;
      case 2:
	do {
	  SetSpyOrTrace(pte, set_flag);
	  trace_printf(*info, "\t%s %s on predicate %s:%s/%d\n",
		       ...
		       mte->name, pte->name, pte->pred->arity, 0, 0);
	  pte++;
	...
	trace_printf(*info, "\t%s %s on predicate %s:%s/%d\n",
		     spy_trace[is_spy], set_reset[set_flag],
		     mte->name, pte->name, pte->pred->arity, 0);
	break;
      }
      break;
    }
  case PrintDepth:
    skip_blanks();
    if (trace_parse_int((long *)&Depth(*info)) != 1) {
      trace_printf(*info, "\tCurrent print depth = %ld\n", Depth(*info), 0, 0, 0, 0, 0);
    }
    break;
  case PrintLength:
    skip_blanks();
    if (trace_parse_int((long *)&Length(*info)) != 1) {
      trace_printf(*info, "\tCurrent print length = %ld\n", Length(*info), 0, 0, 0, 0, 0);
    }
    break;
  case PrintVerbose:
...
static void control_command(command, info)
     enum tracer_command command;
     struct trace_info_rec *info;
{
  switch(command) {
...
static int call_port_command(qp, curmod, info)
     struct goalrec *qp;
     struct mod_table_entry *curmod;
     struct trace_info_rec **info;
{
...
static int reduce_port_command(subgoals, qp, curmod, info)
...
static int susp_port_command(qp, curmod, info)
     struct goalrec *qp;
     struct mod_table_entry *curmod;
     struct trace_info_rec **info;
{
...
static int fail_port_command(qp, curmod, info)
     struct goalrec *qp;
     struct mod_table_entry *curmod;
     struct trace_info_rec **info;
{
...
static void print_port_status(info)
     struct trace_info_rec *info;
{
     ...
     struct trace_info_rec *info;
{
  ...
  trace_printf(info,
	       ...
	       Depth(info), Length(info), 0, 0, 0);
}

static void print_port_names(info)
     struct trace_info_rec *info;
{
...
static void tracer_error(info, s, a0, a1, a2, a3, a4, a5/*, a6, a7, a8*/)
     struct trace_info_rec *info;
     char *s;
...
static void print_help(messages, info)
     Const char **messages;
     struct trace_info_rec *info;
{
  while (*messages) {
...
static void trace_print(out, x, depth, length)
     FILE *out;
     q x;
     unsigned long depth, length;
{
  extern void fprint_partially(FILE *, q, unsigned long, unsigned long);
     ...
     int verbose;
{
  ...
  freopen(tracetmpname,"r",tracetmp);
  {
	...
	io_buf[i] = ch;
      ...
      io_buf[i] = '\0';
      trace_printf(tracetmp, "%s", io_buf, depth, length, 0, 0, 0);
...
void trace_fflush(info)
     struct trace_info_rec *info;
{
  klic_fflush(OutStream(info));
}

void trace_printf(info, format, a0, a1, a2, a3, a4, a5/*, a6, a7, a8, a9, aa, ab*/)
     struct trace_info_rec *info;
     char *format;
...
void trace_fgets(buf, n, info)
     char *buf;
     int n;
     struct trace_info_rec *info;
{
  extern char *klic_fgets(char *, size_t, FILE *);
  klic_fgets(buf, n, InStream(info));
}

void print_priority(prio, info)
     long prio;
     struct trace_info_rec *info;
{
  declare_globals;
  if (prio < current_prio) {
    trace_printf(info, "@prio(-%ld)", current_prio-prio, 0, 0, 0, 0, 0);
  } else if (prio > current_prio) {
    trace_printf(info, "@prio(+%ld)", prio-current_prio, 0, 0, 0, 0, 0);
  }
}

void print_goal(g, mod, info)
     struct goalrec *g;
     module mod;
     struct trace_info_rec *info;
{
...
void fprint_goal(stream, g, mod, depth, length, verbose)
     FILE *stream;
     struct goalrec *g;
     module mod;
     unsigned long depth, length;
...
static void print_modules(info)
     struct trace_info_rec *info;
{
  int k;
  ...
  for (k = 0; k < n_mod; k++) {
    ...
    trace_printf(info,
		 ...
		 (unsigned long)mte->name, 0, 0, 0, 0);
  }
  trace_printf(info, "\t*******************\n", 0, 0, 0, 0, 0, 0);
}

static void print_predicates(mte, pte, arity, spied_only, info)
     struct mod_table_entry *mte;
     struct pred_table_entry *pte;
     int arity;
     int spied_only;
...
{
  if (Traced(qp)) {
    trace_printf(info, "%6d* ", TraceSeq(qp), 0, 0, 0, 0, 0);
  } else {
    trace_printf(info, "%6d  ", dump_seq++, 0, 0, 0, 0, 0);
  }
  print_goal(qp, 0, info);
...
static void print_ready_queue(qp, dump_seq, info)
     struct goalrec *qp;
     long dump_seq;
     struct trace_info_rec *info;
{
...
static void print_suspended_goals(info)
     struct trace_info_rec *info;
{
  declare_globals;
