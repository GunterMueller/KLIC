...
{
  int c;
  // (void)fprintf(file, "(unsigned char *)"); /* ueda */
  (void)fprintf(file, "(" SX(Unsigned) "char *)");
  (void)putc('\"', file);
  while ((c=*str) != 0) {
...
static void write_db_files(inittime)
     time_t inittime;
{
  ...
  if (!nocfiles) {
    ...
    (void)fprintf(atomc, SX(Unsigned) "char * Const init_atomname[] = {\n");
  }
  for (a=atomroot; a!=0; a=a->next) {
  ...
  if (!nocfiles) {
    (void)fprintf(atomc, "};\n");
    (void)fprintf(atomc,
		  "Const unsigned long initial_atoms_atom_c = %d;\n", nextatom);
    // (void)fprintf(atomc, "Unsigned char **atomname_atom_c = (Unsigned char **)init_atomname;\n"); /* ueda */
    (void)fprintf(atomc, SX(Unsigned) "char **atomname_atom_c = (" SX(Unsigned) "char **)init_atomname;\n");
    (void)fclose(atomh);
    (void)fclose(atomc);
  }
  ...
  if (!nocfiles) {
    ...
    (void)fprintf(functc, "};\n");
    (void)fprintf(functc,
		  "Const unsigned long initial_functors_funct_c = %d;\n",
		  nextfunct);
    (void)fprintf(functc,
		  "unsigned long *functors_funct_c = (unsigned long *)init_functors;\n");
    (void)fprintf(functc,
		  "unsigned long *arities_funct_c = (unsigned long *)init_arities;\n");
    (void)fclose(functh);
    (void)fclose(functc);
  }
...
#define Push(nam, mc) { smdp->name = nam; smdp->morc = mc; smdp++; }
	...
	*newlinked = *searched;
	...
	if (m->morc == Mod) {
      ...
      (void)fprintf(predc, "Const struct modinfo defined_modules_predicates_c[] = {\n");
      for (m=linkedmod; m!=0; m=m->next) {
	if (m->morc == Mod) {
	  (void)fprintf(predc, "  ");
	  {
...
int from_same_installation(klicdb, inittime)
     FILE *klicdb;
     time_t inittime;
{
  int c;
     ...
     char *name;
{
  ...
  if (err == 0) return;
  if (errno != ENOENT) {
    error_exit("Can't clear file: %s", name, 0);
  }
}
     ...
     char **argv;
{
  ...
  if (stat(initpath, &initstat) != 0) {
    error_exit("Can't access file: %s", initpath, 0);
  }
  if (!makelib && stat(libdbpath, &libdbstat) != 0) {
    error_exit("Can't access file: %s", libdbpath, 0);
  }
