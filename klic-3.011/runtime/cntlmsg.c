...
extern void add_node_wtc(long);
extern long get_wtc(long, enum get_wtc_type);
extern void message_suspend(long, long, combuf *);
extern long regist_exptbl(q);
/*
extern void
#ifdef USE_STDARG
ioeprintf(char *, ...);
...
#define  diff_usec_dist(field)\
...

  {
      ...
      if (!IS_SHOEN_NODE(my_node)) {
	  sprintf(ptr, " heap size = %lu words\n", heapsize);
	  ptr += strlen(ptr);
      }
      ...
      if (!IS_SHOEN_NODE(my_node)) {
	  if (measure_gc) {
	      sprintf(ptr, " %d ms utime & %d ms stime in ",
		      gcums, gcsms);
	      ptr += strlen(ptr);
	  }
	  sprintf(ptr, " %d GC %ld suspensions; %ld resumptions\n",
