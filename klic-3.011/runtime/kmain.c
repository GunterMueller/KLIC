...
/* Globals referenced from the shared library are now put here 
   and copied from main for successful linking with shared 
   libraries (.so and .dll) */ /* ueda */
struct predicate *predicate_main_xmain_0_p;
unsigned long initial_atoms;
Unsigned char **atomname;
unsigned long initial_functors;
unsigned long *functors;
unsigned long *arities;
struct modinfo *defined_modules;

int klic_main(argc, argv,
	      km_predicate_main_xmain_0_p,
	      km_initial_atoms,
	      km_atomname,
	      km_initial_functors,
	      km_functors,
     ...
     int argc;
     char **argv;
     struct predicate *km_predicate_main_xmain_0_p;
     Const unsigned long km_initial_atoms;
     Unsigned char **km_atomname;
     Const unsigned long km_initial_functors;
     unsigned long *km_functors;
     unsigned long *km_arities;
     struct modinfo *km_defined_modules;
{
  declare_globals;

  // set globals referenced from shared libraries /* ueda */
  predicate_main_xmain_0_p = km_predicate_main_xmain_0_p;
  initial_atoms = km_initial_atoms;
  atomname = km_atomname;
  initial_functors = km_initial_functors;
  functors =  km_functors;
  arities = km_arities;
  defined_modules = km_defined_modules;
  ...
  }
  {
	 ...
	 child processes:	1 <= nodeid <=  total_node+1 */
      extern int r_spawn_children();
      extern void io_init_comm();
      extern void init_dist();
...

  {
    ...
    extern void initialize_suspension_count_table();
    q *allocp;
    struct goalrec *qp;
    extern void register_gc_hook(q*(*)());
    extern void register_after_gc_hook(q*(*)());
    extern int add_signal_handler(int, int(*)());
    extern void init_rusage();
    extern void initiate_trace();
    extern void initialize_suspension_count_table();
    extern void init_dist_signal_setup();
    ...
    if (IS_MASTER_NODE(my_node)) {	/* master node */
        // qp->pred = &predicate_main_xmain_0_kmain_c; /* ueda */
	qp->pred = predicate_main_xmain_0_p;
	heapp = allocp + 2;
...
#ifdef DIST
{
  ...
  extern Const struct predicate topsucceed_pred;
  extern void terminate_all_node();
  extern void close_network();
...
#else /* #ifndef DIST */
{
  extern q *klic_interrupt();
  extern Const struct predicate topsucceed_pred;
  extern void scan_suspended_pred();
