...
if ( test "$PVM_ARCH" = "VPP300" || test "$PVM_ARCH" = "VPP700" ); then
  ...
  DISTCOMPILERFLAG="{(void)sprintf(bufp, \" -DDIST %s -I. -I%s %s%s -Wl,-P -dy -J\", klic_incdir, PVMINC, (name), (ext)); }"
  ...
  DISTCOMPILERFLAG="{(void)sprintf(bufp, \" -DDIST %s -I. -I%s %s%s\", klic_incdir, PVMINC, (name), (ext)); }"
